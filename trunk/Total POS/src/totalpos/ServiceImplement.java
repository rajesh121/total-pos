...
public class ServiceImplement extends java.rmi.server.UnicastRemoteObject implements Services{
    ...
    public void sendCreditNotesReceipt(List<ReceiptSap> creditNotes, List<ReceiptSap> receipts, List<Client> clients) throws RemoteException, SQLException {
        ...
        for (ReceiptSap receiptSap : creditNotes) {
            stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_CAB_DEV ( MANDT , FKDAT , VBELN , WERKS , ZTIPV , "
                    + "KUNNR , RANGO , REPOZ , IMPRE , WAERS ) values ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? ) ");
            ...
            for (Receipt receipt : receiptSap.receipts) {
                for (Item2Receipt item2Receipt : receipt.getItems()) {
                    stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_POS_DEV ( MANDT , FKDAT , VBELN , POSNR , EAN11 , "
                    + "KWMENG , VRKME , CHARG , KBETP , KBETD , PERNR , WERKS ) values ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? ) ");
                    stmt.setString(1, Constants.mant);
                    stmt.setString(2, myDay);
                    stmt.setString(3, "D" + receiptSap.getId());
        ...
        for (ReceiptSap receiptSap : receipts) {
            stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_CAB_FACT ( MANDT , FKDAT , VBELN , WERKS , ZTIPV , "
                    + "KUNNR , RANGO , REPOZ , IMPRE , WAERS ) values ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? ) ");
            stmt.setString(1, Constants.mant);
            stmt.setString(2, myDay);
            ...
            for (Receipt receipt : receiptSap.receipts) {
                ...
                for (Item2Receipt item2Receipt : receipt.getItems()) {
                    stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_POS_FACT ( MANDT , FKDAT , VBELN , POSNR , EAN11 , "
                    + "KWMENG , VRKME , CHARG , KBETP , KBETD , PERNR , WERKS ) values ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? ) ");
                    stmt.setString(1, Constants.mant);
                    stmt.setString(2, myDay);
                    stmt.setString(3, "F" + receiptSap.getId());
        ...
        for (Client client : clients) {
            ...
            stmt.executeUpdate();
            stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_CLIENT ( MANDT , FKDAT , KUNNR , NAME1 , ADRNR , WAERS )"
                    + " values ( ? , ? , ? , ? , ? , ? ) ");
            stmt.setString(1, Constants.mant);
            stmt.setString(2, myDay);
    ...
    public void createDummySeller() throws RemoteException, SQLException {
        PreparedStatement stmt = Constants.connection.prepareStatement("insert into "+Constants.dbHeader+".ZSDS_VEND_FACT ( MANDT , VBELN , POSNR , FKDAT , WERKS , "
                + "PARTN_ROLE , PARTN_NUMB ) values ( ? , ? , ? , ? , ? , ? , ? )");
        stmt.setString(1, Constants.mant);
